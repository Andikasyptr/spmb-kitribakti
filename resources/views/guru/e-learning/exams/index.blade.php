        ...
        <table class="min-w-full border-collapse">
            <thead>
                <tr class="bg-gray-100 text-gray-700">
                    ...
                    <th class="p-3 text-left font-medium border-b">Durasi</th>
            ...
            <tbody>
                @foreach($exams as $exam)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="p-3 border-b text-gray-600">{{ $exam->description ?? '-' }}</td>
                        <td class="p-3 border-b text-gray-800">{{ $exam->duration }} menit</td>
                    ...
                    <td class="p-3 border-b text-orange-600 font-semibold">
                        ...
                        <td class="p-3 border-b">
                            <div class="flex flex-wrap gap-2">
                                ...
                                <form action="{{ route('guru.exams.destroy', $exam->id) }}" method="POST" class="delete-exam-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" 
    ...
    <div class="grid gap-4 md:hidden">
        @foreach($exams as $exam)
            <div class="border rounded-lg p-4 shadow-sm flex flex-col gap-2">
                ...
                <p class="text-sm text-gray-600">Jurusan: {{ $exam->description ?? '-' }}</p>
                <p class="text-sm text-gray-600">Durasi: {{ $exam->duration }} menit</p>
                <p class="text-sm text-gray-600">
                    Mulai: {{ $exam->start_time ? \Carbon\Carbon::parse($exam->start_time)->format('d M Y H:i') : '-' }}
                </p>
                <p class="text-sm text-gray-600">
                    Selesai: {{ $exam->end_time ? \Carbon\Carbon::parse($exam->end_time)->format('d M Y H:i') : '-' }}
                </p>
                ...
                <div class="flex gap-2 mt-2">
                    ...
                    <form action="{{ route('guru.exams.destroy', $exam->id) }}" method="POST" class="flex-1 delete-exam-form">
                        @csrf
                        @method('DELETE')
                        <button type="button" 
