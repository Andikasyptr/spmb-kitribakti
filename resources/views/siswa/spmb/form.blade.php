...
<div class="py-6">
    <div class="max-w-5xl mx-auto bg-white p-6 rounded shadow">
        ...
        @if($sudahTerdaftar)
            <div class="text-center p-6 bg-red-100 rounded">
                <p class="text-red-700 font-semibold">
                    Anda sudah terdaftar dan tidak bisa mengisi pendaftaran 2 kali.
                </p>
            </div>
        @else
            {{-- Form Siswa --}}
            <form action="{{ route('spmb.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <x-input label="NISN" name="nisn" value="{{ old('nisn') }}" maxlength="10" pattern="\d{10}" inputmode="numeric" />
                    <x-input label="NIK" name="nik" value="{{ old('nik') }}" maxlength="16" pattern="\d{16}" inputmode="numeric" />
                    ...
                    <x-input label="No. KK" name="no_kk" value="{{ old('no_kk') }}" maxlength="16" pattern="\d{16}" inputmode="numeric" />
                    ...
                    <div>
                        ...
                        <select name="jurusan" class="w-full mt-1 p-2 border rounded">
                            ...
                            @foreach($jurusans as $jurusan)
                                <option value="{{ $jurusan->nama_jurusan }}" {{ old('jurusan', $siswa->jurusan ?? '') == $jurusan->nama_jurusan ? 'selected' : '' }}>
                    ...
                    <x-input label="No. HP" name="no_hp" value="{{ old('no_hp') }}" maxlength="13" pattern="\d{10,13}" inputmode="numeric" />

                    {{-- Data Ayah --}}
                    <x-input label="Nama Ayah" name="nama_ayah" value="{{ old('nama_ayah') }}" />
                    <x-input label="NIK Ayah" name="nik_ayah" value="{{ old('nik_ayah') }}" maxlength="16" pattern="\d{16}" inputmode="numeric" />
                    <div>
                    ...
                    <x-input label="NIK Ibu" name="nik_ibu" value="{{ old('nik_ibu') }}" maxlength="16" pattern="\d{16}" inputmode="numeric" />
                    <div>
                    ...
                    <div class="md:col-span-2 border-t pt-4 mt-4">
                        ...
                        {{-- File NISN --}}
                        <x-input label="File NISN" type="file" name="file_nisn" />
                        ...
                        {{-- Akta Kelahiran --}}
                        <x-input label="File Akta Kelahiran" type="file" name="file_akta" />

                        <p class="text-sm text-gray-500 mb-1">* untuk foto hanya file JPG yang diperbolehkan</p>
...
document.addEventListener('DOMContentLoaded', function() {
    Swal.fire({
        icon: 'error',
        title: 'Terdaftar!',
        text: 'Anda sudah terdaftar, mohon selesaikan administrasi pembayaran sekarang.',
